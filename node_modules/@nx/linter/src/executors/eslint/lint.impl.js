...
function run(options, context) {
    ...
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        options.cacheLocation = options.cacheLocation
            ...
            : undefined;
        const { printConfig } = options, normalizedOptions = tslib_1.__rest(options, ["printConfig"]);
        /**
        ...
        const { eslint, ESLint } = yield (0, eslint_utils_1.resolveAndInstantiateESLint)(eslintConfigPath, normalizedOptions, useFlatConfig);
        ...
        if (printConfig) {
            try {
                const fileConfig = yield eslint.calculateConfigForFile(printConfig);
        ...
        let lintResults = [];
        try {
            lintResults = yield eslint.lintFiles(normalizedOptions.lintFilePatterns);
        }
        catch (err) {
...
`);
        ...
        if (lintResults.length === 0) {
            const ignoredPatterns = (yield Promise.all(normalizedOptions.lintFilePatterns.map((pattern) => tslib_1.__awaiter(this, void 0, void 0, function* () { return (yield eslint.isPathIgnored(pattern)) ? pattern : null; }))))
        ...
        const formatter = yield eslint.loadFormatter(normalizedOptions.format);
        let totalErrors = 0;
        let totalWarnings = 0;
